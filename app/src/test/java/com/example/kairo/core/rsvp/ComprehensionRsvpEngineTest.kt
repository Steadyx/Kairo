...
class ComprehensionRsvpEngineTest {
    ...
    fun longWordsNeverFlashTooFastAtHighWpm() {
        ...
        assertTrue(
            "Expected >= ${config.longWordMinMs}ms, got ${frames[0].durationMs}ms",
            frames[0].durationMs >= config.longWordMinMs
        )
    }
